...
// OpenRouter API configuration - Keys loaded from environment variables (SECURE)
// Keys are set in app.config.js and loaded from .env file
const OPENROUTER_API_KEYS = [
  Constants.expoConfig?.extra?.OPENROUTER_API_KEY,
  Constants.expoConfig?.extra?.OPENROUTER_API_KEY_BACKUP_1,
  Constants.expoConfig?.extra?.OPENROUTER_API_KEY_BACKUP_2,
...
function getApiKey(): string {
  ...
  if (OPENROUTER_API_KEYS.length === 0) {
    ...
    throw new Error(
      'OpenRouter API keys not configured. Please:\n' +
      '1. Copy .env.example to .env\n' +
      '2. Add your API key from https://openrouter.ai/keys\n' +
      '3. Restart the app'
    );
  }
...
Begin analysis now:`;
    ...
    // Create AbortController with 60 second timeout (vision models can be slow)
    ...
    try {
      ...
      return await handleApiResponse(response, imageUri, model);
    } catch (error) {
      clearTimeout(timeoutId);
...
async function handleApiResponse(response: Response, imageUri: string, model: string): Promise<MealScan> {
  if (!response.ok) {
